  ...
  fs.readdir(uploadDir, function (err, list) {
    if(list && list.length){
      list.forEach(function (name) {
        var stats = fs.statSync(uploadDir + '/' + name);
        if (stats.isFile() && name[0] !== '.') {
          html += "<tr><td>"+(++count)+"</td><td><img width='150px' src='"+baseUrl + name+"' title='"+name+"' /></td><td>"+stats.ctime.toISOString().replace('T',' ').replace('.000Z','')+"</td><td>"+humanFileSize(stats.size)+"</td></tr>";
        }else{
