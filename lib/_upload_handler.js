...
UploadHandler.prototype.getList = function () {
  var handler = this,
    ...
    bucket = query.bucket?query.bucket:'default',
  ...
  fs.readdir(uploadDir, function (err, list) {
    if(list && list.length){
      list.forEach(function (name) {
        var stats = fs.statSync(uploadDir + '/' + name);
        if (stats.isFile() && name[0] !== '.') {
          html += "<tr><td>"+(++count)+"</td><td><img width='150px' src='"+baseUrl + encodeURIComponent(name)+"' /></td><td>"+stats.ctime.toISOString().replace('T',' ').replace('.000Z','')+"</td><td>"+humanFileSize(stats.size)+"</td></tr>";
        }
      });
      html += "</table>";
    }
  ...
  if(url_sp.length == 4 && url_sp[1] == "res"){
    ...
    readFile (path, function(err, data){
      if(err){
        ...
        handler.res.writeHead(200, {'Content-Type': 'image/jpeg'});
        handler.res.end(data);
      }
    });
    return;
